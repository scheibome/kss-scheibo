...
const replaceVitePugTags = require('../helpers/replaceVitePugTag');
...
module.exports = (Handlebars, options) => {
	Handlebars.registerHelper('modifierInsertSection', (markup, sections, styleGuide, sourcePath) => {
		...
		let output = replaceVitePugTags(markup, options);

		// Init function
		const insertLines = output.match(regexModifierLine);
		// Check isset '<insert-markup>'
		if (!insertLines) {
			return output;
		}

		return replaceInsert(output, styleGuide);
	});
};
