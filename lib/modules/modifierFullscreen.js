...
// get the config setup from cli
if (process.argv.indexOf('--config') >= 0) {
	let configCliPosition = process.argv.indexOf('--config') + 1;
	settings = JSON.parse(fs.readFileSync('./' + process.argv[configCliPosition]));
} else {
	// set cli settings if not config-file isset
	let requiredVars = ['css', 'js', 'destination'];

	requiredVars.forEach(function(el) {
		'use strict';
		if (process.argv.indexOf('--' + el) >= 0) {
			settings[el] =  process.argv[process.argv.indexOf('--' + el) + 1];
		}
	});
}
