...
function addClassToHtmlTag(html, className) {
	if (!className) {
		return html;
	}

	// regex, to find the <html> tag
	const htmlTagRegex = /<html([^>]*)>/i;

	// replace the <html> tag to add the class
	const updatedHtml = html.replace(htmlTagRegex, (match, p1) => {
		// check if the class attribute is already present
		if (/class\s*=\s*"/i.test(p1)) {
			return match.replace(/class\s*=\s*"/i, `class="${className} `);
		} else {
			return `<html${p1} class="${className}">`;
		}
	});
...
module.exports = function(Handlebars, options) {
	...
	Handlebars.registerHelper('modifierFullscreenButton', function(
		...
		let html = createHTML({
			...
			`,
			favicon: 'kss-assets/kss-favicon.svg',
		...
		html = addClassToHtmlTag(html, htmlclass);
